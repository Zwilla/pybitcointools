...
def mnemonic_int_to_words(mint, mint_num_words, wordlist=None):
    if wordlist is None:
...
def entropy_to_words(entbytes, wordlist=None):
    if wordlist is None:
        wordlist = wordlist_english
...
def words_bisect(word, wordlist=None):
    if wordlist is None:
        wordlist = wordlist_english
...
def words_split(wordstr, wordlist=None):
    if wordlist is None:
        wordlist = wordlist_english

    def popword(pwordstr, pwordlist):
        for fwl in range(1, 9):
            w = pwordstr[:fwl].strip()
            lo, hi = words_bisect(w, pwordlist)
            if hi - lo == 1:
                return w, pwordstr[fwl:].lstrip()
            pwordlist = pwordlist[lo:hi]
...
def words_to_mnemonic_int(words, wordlist=None):
    if wordlist is None:
        wordlist = wordlist_english
...
def words_verify(words, wordlist=None):
    if wordlist is None:
        wordlist = wordlist_english
    if isinstance(words, str):
...
def seed_prefix(sp_seed_type):
    if sp_seed_type == 'standard':
        return SEED_PREFIX
    elif sp_seed_type == 'segwit':
        return SEED_PREFIX_SW
    elif sp_seed_type == '2fa':
...
def words_mine(prefix, entbits, satisfunction, wordlist=None, randombits=random.getrandbits):
    if wordlist is None:
        wordlist = wordlist_english
